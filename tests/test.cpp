...
static void end_test(const char* test_name)
{
	std::cout << std::endl
			  << "########### Ending " << test_name << " ###########" << std::endl
			  << std::endl;
}

static void basic_color_test()
{
...
static void file_test()
{
	...
	logger::warn("But be careful, as currently it also writes m_output_settings as well, however this should not be a problem if you intend to cat the file in your terminal");

	uint32_t data = 123456789;
	logger::notify("Some important data: ", data);
	logger::warn("trying 'cat log.txt' and you should see the m_output_settings work, now try editing it, you should see that its a bit messy", logger::endl);

	logger::info("The following line will only be outputted to the file");
	temp_logger << "Temporary output";
}

static void multiple_streams()
{
...
static void new_line_test()
{
	logger::info("Looks some text");
	logger::new_line();
	logger::notify("THAT SHOULD've been one new line, without flushing buffer");
	logger::new_line(10);
	logger::notify("That's 10 new lines without flushing a buffer between each one");
}
